...
<div class="form">
	...
	<div class="row">
		...
		<?php echo $form->textField($model, 'diary_number', array(
			'size' => 60,
			'maxlength' => 255,
			'disabled' => $model->diary_number ? true : false,
		)
		); ?>
	...
	<div class="row">
		...
		<?php echo $form->textField($model, 'reference_number', array(
			'size' => 60,
			'maxlength' => 255,
			'disabled' => $model->reference_number ? true : false,
		)
		); ?>
	...
	<div class="row">
		...
		<?php
		if ($model->date_of_document) {
			echo $form->textField($model, 'date_of_document', array(
				'disabled' => true,
			)
			);
		} else {
			$this->widget('zii.widgets.jui.CJuiDatePicker', array(
				...
				'htmlOptions' => array(
					'style' => 'height:20px;',
					// 'disabled' => $model->date_of_document ? true : false,
				),
			)
			);
		}
	...
	<div class="row">
		...
		$this->widget('zii.widgets.jui.CJuiAutoComplete', array(
			...
			'htmlOptions' => array(
				'size' => '60',
				'disabled' => $model->received_from ? true : false,
			),
		)
		);
		?>
	...
	<div class="row">
		...
		<?php echo $form->textArea($model, 'description', array(
			'rows' => 6,
			'cols' => 50,
			'disabled' => $model->description ? true : false,
		)
		); ?>
	...
	<div class="row">
		...
		<?php $this->widget('application.extensions.multicomplete.MultiComplete', array(
			...
			'htmlOptions' => array(
				'size' => '60',
				'disabled' => $model->doc_status ? true : false,
			),
		)
		);
		?>
	...
	<div class="row">
		...
		<?php $this->widget('application.extensions.multicomplete.MultiComplete', array(
			...
			'htmlOptions' => array(
				'size' => '60',
				'disabled' => $model->type_of_document ? true : false,
			),
		)
		);
		?>
	...
	<div class="row">
		...
		<?php echo $form->textField($model, 'detail_of_enclosure', array(
			'size' => 60,
			'maxlength' => 255,
			'disabled' => $model->detail_of_enclosure ? true : false,
		)
		); ?>
	...
	<div class="row">
		...
		<?php $this->widget('application.extensions.multicomplete.MultiComplete', array(
			...
			'htmlOptions' => array(
				'size' => '60',
				'disabled' => $model->tags ? true : false,
			),
		)
		);
		?>
	...
	<div class="row">
		<?php echo 'Marked by'; ?>
		<?php echo $form->dropDownList($model, 'owner_id', Assignments::getDesignationsListIndexedByDutyID(), array(
			'disabled' => $model->owner_id ? true : false,
		)
		); ?>
	...
	<?php if ($model->isNewRecord != '1') { ?>
		<div class="row">
			<?php foreach ($model->images as $key => $image) {
				...
				echo CHtml::image(Yii::app()->baseUrl . '/document_images/' . $image->image_path, $image->image_path, array("width" => 200)); // Image shown here if page is update page
				if (!$image->id)
