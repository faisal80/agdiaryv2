 ...
 */
class Assignments extends DiaryActiveRecord
{
	...
	public static function getDesignationsListIndexedByDutyID()
	{
		...
		foreach ($_officers as $_officer) {
			...
			$_assignments = Assignments::model()->findAll('officer_id=:officer_id AND state=1', array(':officer_id' => $_officer->officer_id));
	 ...
	 */
	public static function getDesignationsListIndexedByDutyID1($short = false)
	{
		$result = array();
		//Find all active duty assignments
		$_assignments = Assignments::model()
									->with(array('designation'=>array(
											'select'=>false,
											'order'=>'level')
											))
									->findAll('state=1');
		foreach ($_assignments as $_assignment) {
			if ($short) {
				$result = $result + array($_assignment->duty->id => $_assignment->designation->short_form . ' (' . $_assignment->duty->short_form . ')');
			} else {
				$result = $result + array($_assignment->duty->id => $_assignment->designation->title . ' (' . $_assignment->duty->duty . ')');
			}
		}
	...
	public function getFullAssignment($short_form = false)
	{
		if ($short_form)
			return $this->designation->short_form . (!empty($this->duty->short_form) ? ' (' . $this->duty->short_form . ')' : '');
		return $this->designation->title . (!empty($this->duty->short_form) ? ' (' . $this->duty->short_form . ')' : '');
	}

	public function getShortAssignment()
	{
		return $this->designation->short_form . (!empty($this->duty->short_form) ? ' (' . $this->duty->short_form . ')' : '');
