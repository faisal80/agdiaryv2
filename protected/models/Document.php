 ...
 * @property boolean $is_case_disposed
 ...
 * @property string $page_no_in_file
 */
class Document extends DiaryActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			((Yii::app()->user->canCreateBlankDoc()) ?
				array('reference_number, date_of_document, received_from, description', 'safe') :
				array('reference_number, date_of_document, received_from, description', 'required')),
			array('diary_number', 'length', 'max' => 30),
			array('page_no_in_file', 'length', 'max' => 20),
			array('owner_id', 'safe'),
			...
			array('is_case_disposed', 'default', 'value' => false),
			...
			array('id, diary_number, date_received, reference_number, date_of_document, received_from, description, owner_id, doc_status, is_case_disposed, type_of_document, detail_of_enclosure, tags, create_time, create_user, update_time, update_user, page_no_in_file', 'safe', 'on' => 'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'lastMarkingR' => array(self::HAS_MANY, 'Marked', 'document_id', 'order' => 'create_time DESC', 'limit' => 1),
			'file_movements' => array(self::HAS_MANY, 'FileMovement', 'document_id', 'order' => 'id DESC', 'limit' => 1),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('page_no_in_file', $this->page_no_in_file, true);
	...
	protected function beforeSave()
	{
		...
		if (!empty($this->date_of_document)) {
			if (stripos($this->date_of_document, Yii::app()->user->getDateSeparator()) == 2)
				$this->fixDate($this, 'date_of_document');
		}
	...
	{
		if (!stristr($this->doc_status, 'confidential') == false)
			return true;
		if (!stristr($this->doc_status, 'secret') == false)
			return true;
		return false;
	}
	...
	/**
	 * This function returns second last marking of the document.
	 * @return Marked|null instance of last marking or null
	 */
	...
	public function SecondLastMarking()
	{
		...
		if (!empty($secondLastMarking[1]))
			return $secondLastMarking[1];
		return null;
	}

	/**
	 * This funtion checks whether all required fields are filled or not
	 * @return boolean
	 */
